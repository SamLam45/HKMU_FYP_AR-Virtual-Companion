...
class ArView(
    ...
    private val onObjectMethodCall =
        MethodChannel.MethodCallHandler { call, result ->
            when (call.method) {
                ...
                "rotateNodeY" -> handleRotateNodeY(call, result)
                "setNodeLocalOffset" -> handleSetNodeLocalOffset(call, result)
                "setNodeYawAndLocalOffset" -> handleSetNodeYawAndLocalOffset(call, result)
                else -> result.notImplemented()
            }
        }
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        val type = nodeData["type"] as? Int ?: return null
        var fileLocation = nodeData["uri"] as? String ?: return null
        // Types 2–3: app_flutter 內 GLB。不可用 raw 絕對路徑 — SceneView 會當 asset 開。
        // 亦勿用 createModelInstance(File)：同步 loadResources 曾於 gltfio createTextures SIGSEGV。
        // 用 suspend loadModelInstance(Uri.fromFile(f).toString())（見 sceneview-android #263）。
        var localModelFile: File? = null
        when (type) {
            ...
            2, 3 -> { // uri relative to Flutter getApplicationDocumentsDirectory() → PathUtils.getDataDirectory() (app_flutter)
                val relative = fileLocation.trimStart('/')
                val docRoot = PathUtils.getDataDirectory(viewContext.applicationContext)
                val file = File(docRoot, relative)
                if (!file.isFile || file.length() == 0L) {
                    Log.w(
                        TAG,
                        "local model missing or empty: docRoot=$docRoot relative=$relative " +
                            "abs=${file.absolutePath} exists=${file.exists()} len=${file.length()}",
                    )
                    return null
                }
                Log.i(
                    TAG,
                    "local GLB: abs=${file.absolutePath} len=${file.length()}",
                )
                localModelFile = file
            }
            else -> return null
        ...
        return try {
            val modelInstance: ModelInstance =
                if (localModelFile != null) {
                    ...
                    Log.i(
                        TAG,
                        "local GLB loadModelInstance(file://): $fileUri len=${f.length()}",
                    )
                    sceneView.modelLoader.loadModelInstance(fileUri) ?: run {
                        Log.e(TAG, "loadModelInstance null for: $fileUri")
                        return null
                    }
                } else {
                    sceneView.modelLoader.loadModelInstance(fileLocation) ?: run {
                        Log.e(TAG, "loadModelInstance null for: $fileLocation")
                        return null
                    }
                }
            object : ModelNode(
                ...
                mainScope.launch {
                    try {
                        buildModelNode(dict_node)?.let { node ->
                            ...
                            // 只在 anchorNode 尚未加入 scene 時才呼叫 addChildNode，
                            // 避免重複加入時移除再插入，導致 AR 錨點位置/旋轉被重置。
    ...
    try {
        ...
        nodesMap[name]?.let { node ->
            newTransformation?.let { transform ->
                ...
                // SceneRotation 接受 degrees，atan2 返回 radians，需轉換
                val toDeg = (180.0 / kotlin.math.PI).toFloat()
                    ...
                    node.apply {
                        transform(
                            ...
                            rotation = SceneRotation(
                                x = toDeg * kotlin.math.atan2(transform[6].toFloat(), transform[10].toFloat()),
                                y = toDeg * kotlin.math.atan2(-transform[2].toFloat(),
                                    kotlin.math.sqrt(transform[6].toFloat() * transform[6].toFloat() +
                                    transform[10].toFloat() * transform[10].toFloat())),
                                z = toDeg * kotlin.math.atan2(transform[1].toFloat(), transform[0].toFloat())
                            ),
                            scale = SceneScale(
    ...
    /**
     * 直接將指定節點繞 Y 軸旋轉至 [yawDeg] 度（degrees）。
     * 旋轉目標為父節點（AnchorNode），因為動畫 GLB 每幀會覆蓋 ModelNode 本身的旋轉，
     * 但 AnchorNode 不受動畫系統控制，旋轉效果才能持久生效。
     */
    private fun handleRotateNodeY(call: MethodCall, result: MethodChannel.Result) {
        try {
            val name = call.argument<String>("name")
            val yawDeg = call.argument<Double>("yawDeg") ?: 0.0
            if (name == null) {
                result.error("INVALID_ARGUMENT", "Node name is required", null)
                return
            }
            val modelNode = nodesMap[name]
            if (modelNode == null) {
                result.error("NODE_NOT_FOUND", "Node $name not found", null)
                return
            }
            mainScope.launch {
                // 優先旋轉父節點（AnchorNode），動畫不會覆蓋父節點的旋轉
                val targetNode: Node = modelNode.parent ?: modelNode
                targetNode.rotation = SceneRotation(0f, yawDeg.toFloat(), 0f)
                Log.d(TAG, "rotateNodeY: name=$name yaw=$yawDeg target=${targetNode.javaClass.simpleName}")
            }
            result.success(null)
        } catch (e: Exception) {
            Log.e(TAG, "handleRotateNodeY error", e)
            result.error("ROTATE_NODE_Y_ERROR", e.message, null)
        }
    }

    /**
     * 設定 ModelNode 相對於 AnchorNode 的 X/Z 位置偏移（Y 保持不變）。
     * 搖桿移動角色時呼叫此方法，位移在 AnchorNode 的本地空間，
     * 即當前面向方向為 -Z，橫向為 X。
     */
    private fun handleSetNodeLocalOffset(call: MethodCall, result: MethodChannel.Result) {
        try {
            val name = call.argument<String>("name")
            ...
            mainScope.launch {
                val currentY = modelNode.position.y
                modelNode.position = ScenePosition(x.toFloat(), currentY, z.toFloat())
            }
            result.success(null)
        } catch (e: Exception) {
            Log.e(TAG, "handleSetNodeLocalOffset error", e)
            result.error("SET_OFFSET_ERROR", e.message, null)
        }
    }

    /**
     * 原子設定 AnchorNode 的 Y 軸旋轉角度，以及 ModelNode 相對於 AnchorNode 的 X/Z 位置。
     * 兩個操作在同一個 coroutine 內完成，避免先轉後移之間的一幀跳動（race condition）。
     */
    private fun handleSetNodeYawAndLocalOffset(call: MethodCall, result: MethodChannel.Result) {
        try {
            val name = call.argument<String>("name")
            val yawDeg = call.argument<Double>("yawDeg") ?: 0.0
            val x = call.argument<Double>("x") ?: 0.0
            ...
            mainScope.launch {
                val anchorNode: Node = modelNode.parent ?: modelNode
                anchorNode.rotation = SceneRotation(0f, yawDeg.toFloat(), 0f)
                // 清除 ModelNode 自身的旋轉，確保旋轉只由 AnchorNode 承載。
                // 避免 Dart transformationChanged 或初始放置殘留的 rotation 與 AnchorNode 疊加。
                modelNode.rotation = SceneRotation(0f, 0f, 0f)
                val currentY = modelNode.position.y
                modelNode.position = ScenePosition(x.toFloat(), currentY, z.toFloat())
            }
            result.success(null)
        } catch (e: Exception) {
            Log.e(TAG, "handleSetNodeYawAndLocalOffset error", e)
            result.error("SET_YAW_OFFSET_ERROR", e.message, null)
        }
    }
